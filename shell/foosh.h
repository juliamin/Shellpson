...
typedef struct process_t
{
    int pid;        /* Process ID */
    int status;     /*  */
    int completed, stopped;
    char** argv;    /* for exec. This is where the command and its arguments are stored */ 
    int narguments; /* number of arguments */
    struct process_t* next; /* Pointer to the next process */
} process_t;
...
typedef struct pipeline_t
{
    ...
    int fIn, fOut;                  /* File descriptorss */
    ...
    process_t* first_process;       /* First process from the process list */
    struct pipeline_t* next;        /* Next active job */
    pid_t group_pid;                /* Process group pid, pgid */
} pipeline_t;
